    ...
    refreshContent(remove) {
        ...
        utils.api.cart.getContent(options, (err, response) => {
            ...
            $(`[data-cart-itemid='${this.$activeCartItemId}']`, this.$cartContent)
                ...
                .trigger('focus');
                if(document.querySelector('.msrp-strike') !== null) {
...
/* Cart Total Discount Section */

// price selectors
const cartMSRP = '.msrp-strike'
const cartPurchasePrice = '.cart-main-price'
const cartCouponPrice = '.cart-coupon-total'

// get various price arrays (MSRP, purchase price, coupon discount)
function getCartPrices(elem) {
    let discount = document.querySelectorAll("" + elem + "");
    return Array.prototype.map.call(discount, function(node) {
        let price = node.textContent.replace(/\$|-|/gi, "");
...
// add prices in array
function calculateTotalDiscount(arr){
  let totalDiscount = 0;
  for (let i = 0; i < arr.length; i++) {
      totalDiscount += parseFloat(arr[i]);
  }
  return totalDiscount;
}

// factor qty into total discount
var itemQty = [];
function checkQty(){
  let elms = document.querySelectorAll('.cart-qty-input-discount')
  for(let i=0; i<elms.length; i++)
    {
        itemQty.push(elms[i].value)
    }
}

// multiple item price or discount by qty
function multiplyQty(arr, results) {
    for (var i=0; i < arr.length; i++) {
        results.push(arr[i] * itemQty[i])
    }
}
...
function displayTotalDiscount(){
    const displayContainer = document.getElementById('grand-total-saved');
    let arrayAmount = getCartPrices(cartMSRP);
    let arrayStandard = getCartPrices(cartPurchasePrice);
    let totalSaved = 0;

    // get correct qty for each item
    checkQty();
    multiplyQty(arrayAmount, msrpPrices);
    multiplyQty(arrayStandard, standardPrices);

    // check for coupon level discounts
    if(document.querySelector('.cart-coupon-total') !== null){
        let arrayDiscount = getCartPrices(cartCouponPrice);
        totalSaved = parseFloat(calculateTotalDiscount(msrpPrices) + calculateTotalDiscount(arrayDiscount)).toFixed(2);
        arrayDiscount = [];
    } else {
        totalSaved = parseFloat(calculateTotalDiscount(msrpPrices)).toFixed(2);
    }
    
    // print total to cart screen 
    let totalPrice = parseFloat(calculateTotalDiscount(standardPrices)).toFixed(2);
    let displayAmount = document.createTextNode('You Save: $' + parseFloat(totalSaved - totalPrice).toFixed(2));
    displayContainer.appendChild(displayAmount);

    // clear arrays to avoid duplicate values w/ cart changes
    arrayAmount = [];
    msrpPrices = [];
    standardPrices = [];
    itemQty = [];
