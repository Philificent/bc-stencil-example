...
/* trigger menu on hover for desktop */
const $navLi = document.querySelectorAll("#navList .navPages-item");
let viewportWidth;

function openNav(event) {
    if(viewportWidth >= 800) {
       let navDiv = event.target.getElementsByTagName('div')[0];
       let navA = event.target.getElementsByTagName('a')[0];
       navDiv.classList.add('is-open');
       navA.classList.add('is-open');
    }
}
function closeNav(event) {
    if(viewportWidth >= 800) {
        let navDiv = event.target.getElementsByTagName('div')[0];
        let navA = event.target.getElementsByTagName('a')[0];
        navDiv.classList.remove('is-open');
        navA.classList.remove('is-open');
    }
}

function hoverNav() {
    viewportWidth = window.innerWidth || document.documentElement.clientWidth;
        $navLi.forEach(item => item.addEventListener("mouseenter", openNav));
        $navLi.forEach(item => item.addEventListener("mouseleave", closeNav));
}

window.addEventListener("resize", hoverNav);
window.addEventListener("load", hoverNav);