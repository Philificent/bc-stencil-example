...
Built from Stencil's Cornerstone theme for VIP.

## Getting Started
1. `npm install`
2. Install Stencil `npm install -g @bigcommerce/stencil-cli`
3. Connect store with `stencil init` (requires store url, access token, and API host)
4. Run locally with `stencil start`

### Stencil Utils
[Stencil-utils](https://github.com/bigcommerce/stencil-utils) is our supporting library for our events and remote interactions. To install Stencil, read the documentation [here](https://developer.bigcommerce.com/stencil-docs).
